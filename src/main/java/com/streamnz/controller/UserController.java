...
@Tag(name = "User Management", description = "Admin API endpoints for user management，Admin role required")
@Validated
public class UserController {
    ...
    public ResponseEntity<ResponseVO<PageVO<UserVO>>> getUsers(
        ...
        return ResponseEntity.ok(ResponseVO.success("Users queried successfully", pageVO));
    }
    ...
    public ResponseEntity<ResponseVO<PageVO<UserVO>>> pageQuery(@Valid @RequestBody UserQueryDTO queryDTO,
                                                                @Parameter(description = "Current page number, starting from 1")
            @RequestParam(defaultValue = "1") Long current,
                                                                @Parameter(description = "Number of records per page")
            @RequestParam(defaultValue = "10") Long size) {
    ...
    public ResponseEntity<ResponseVO<UserVO>> getUserById(@PathVariable Long id) {
        ...
        List<Role> roles = roleMapper.findRolesByUserId(user.getId());
        String[] roleArr = roles.stream().map(Role::getName).toArray(String[]::new);
        return ResponseEntity.ok(ResponseVO.success("User information retrieved successfully", new UserVO(user, roleArr)));
    }
    ...
    public ResponseEntity<ResponseVO<UserVO>> createUser(@Valid @RequestBody UserCreateDTO createDTO) {
        User createdUser = userService.createUser(createDTO);
        List<Role> roles = roleMapper.findRolesByUserId(createdUser.getId());
        String[] roleArr = roles.stream().map(Role::getName).toArray(String[]::new);
        return ResponseEntity.status(HttpStatus.CREATED).body(ResponseVO.success("User created successfully", new UserVO(createdUser, roleArr)));
    }
    ...
    public ResponseEntity<ResponseVO<UserVO>> updateUser(@Valid @RequestBody UserUpdateDTO updateDTO) {
        ...
        String[] roleArr = roles.stream().map(Role::getName).toArray(String[]::new);
        return ResponseEntity.ok(ResponseVO.success("User updated successfully", new UserVO(updatedUser, roleArr)));
    }

    @DeleteMapping("/{id}")
    @Operation(summary = "Delete user", description = "Deletes a user by their ID")
    public ResponseEntity<ResponseVO<Void>> deleteUser(@PathVariable Long id) {
        boolean deleted = userService.deleteUser(id);
        if (!deleted) {
            return ResponseEntity.notFound().build();
        }
        return ResponseEntity.ok(ResponseVO.success("User deleted successfully", null));
    }
}
